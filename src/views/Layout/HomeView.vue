...
const fetchNotifications = async () => {
  ...
  const { data } = await supabase.from('notifications').select('*, appointments(student_name, appointment_date, appointment_time)').eq('mentor_id', currentUserId.value).eq('status', 'pending')
  if (data) notifications.value = data
}

const acceptAppointment = async (notif) => {
  try {
    await supabase.from('appointments').update({ status: 'accepted' }).eq('id', notif.appointment_id)
...
const rejectAppointment = async (notif) => {
  try {
    await supabase.from('appointments').update({ status: 'rejected' }).eq('id', notif.appointment_id)
      ...
      <v-list-item v-for="notification in notifications" :key="notification.id">
        <v-list-item-content>
          ...
          <v-list-item-subtitle class="text-wrap">
            ...
            {{ notification.appointments.appointment_date }} {{ notification.appointments.appointment_time }}
          </v-list-item-subtitle>

          <div class="d-flex gap-2 mt-2">
            <v-btn small color="primary" @click="acceptAppointment(notification)">Accept</v-btn>
            <v-btn small color="error" @click="rejectAppointment(notification)">Reject</v-btn>
          </div>
        </v-list-item-content>
      </v-list-item>
